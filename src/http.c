...
void http_stats_callback(struct evhttp_request *request, void *data) {
	...
	evbuffer_add_printf(buffer, "PAGE /add %i\n", this->stats.add);
...
void http_add_callback(struct evhttp_request *request, void *data) {
	Http *this = (Http *) data;
	this->stats.add++;
	...
	tree_add(tree, arg_str, arg_str_real, value);
...
void http_suggest_callback(struct evhttp_request *request, void *data) {
	...
	//php output
		...
		for (i = 0; i < count; i++) {
			...
			int value = fetcher->items[i].value;
			evbuffer_add_printf(buffer, "i:%i;a:2:{s:3:\"str\";s:%li:\"%s\";s:5:\"value\";i:%i;}", i, strlen(str), str, value);
		}
...
void http_start(Http *this, char const *addr, int port) {
	...
	evhttp_set_cb(server, "/add", http_add_callback, this);
